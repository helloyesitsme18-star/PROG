...
def aantal_kluizen_vrij():
    ...
    with open('kluizen.txt', 'r') as bestand:
        regels = len(bestand.readlines())
        kluizen_vrij = 12 - regels
...
def nieuwe_kluis():
    ...
    with open('kluizen.txt', 'r') as bestand:
        ...
        for regel in regels:
            kluisnr, pin = regel.strip().split(';')
            bezet.append(int(kluisnr))
            kluizen.remove(int(kluisnr))

    if not kluizen:                 #als de lijst kluizen leeg is, geeft hij foutcode -2
        return -2


    kluiscode = input("Voer een kluiscode in (minimaal 4 tekens, geen ';'): ")
    if len(kluiscode) < 4 or ';' in kluiscode:          # checkt op beide eisen en geeft foutcode -1 indien eentje niet klopt
        return -1

    kluisnummer = kluizen[0]
    with open('kluizen.txt', 'a') as bestand:
        bestand.write(f'{kluisnummer};{kluiscode}\n')
...
def kluis_openen():
    ...
    """
    invoer = input('Vul hier uw kluisnummer in: ')
    invoer_kluiscode = input('Vul uw code in: ')

    with open('kluizen.txt', 'r') as bestand:
        regels = bestand.readlines()
        for regel in regels:
            kluisnr, pin = regel.strip().split(';')
            if invoer == kluisnr and invoer_kluiscode == pin:     #checkt of input combinatie overeenkomt met de lijst
                return True
...
def development_code():
    ...
    if int(keuze) == 1:
        aantal_vrij = aantal_kluizen_vrij()
        print(f'Er zijn nog {aantal_vrij} kluizen beschikbaar.')

    if int(keuze) == 2:
        kluisnummer = nieuwe_kluis()
        if kluisnummer == -1:
            print('Ongeldige invoer, probeer opnieuw.')
            return
        if kluisnummer == -2:
            print('Alle kluizen zijn vol. Onze excuses!')
            return
        else:
            print(f'U heeft kluisnummer: {kluisnummer}')

    if int(keuze) == 3:
        if kluis_openen():
            print('Gelukt! Uw kluis wordt geopend.')
        else:
            print('Onjuiste combinatie, probeer opniew.')
