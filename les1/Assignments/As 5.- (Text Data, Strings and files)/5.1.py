...
def gemiddelde():
    ...
    # zin = zin.split(':')                #zorgt ervoor dat het split gebaseerd op gekozen string, in dit geval (:) alles eromheen wordt gesplitst
    for woord in zin:
        gem_lengte = (gem_lengte + len(woord))
        print()
    gem_lengte = gem_lengte / len(zin)
    return gem_lengte
