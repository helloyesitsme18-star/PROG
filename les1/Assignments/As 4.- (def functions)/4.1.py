...
def som(getal1, getal2, getal3):
    """
    Tel de drie getallen bij elkaar op.

    Args:
        getal1 (int): Het eerste getal van de som
        getal2 (int): Het tweede getal van de som
        getal3 (int): Het derde getal van de som
    Returns:
        int: De som (optelling) van de parameters
    """
    result = getal1 + getal2 + getal3
    return result
...
def development_code():
    ...
    print("resultaat van optelling:", som(30, 30, 30))
    ...
    for i in range(-10, 10):
        for j in range(0, 10):
            for k in range(20, 30):
                __my_assert_args(som, (i,j,k), i+j+k)
