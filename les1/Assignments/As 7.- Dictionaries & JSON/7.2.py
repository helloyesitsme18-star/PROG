...
def tickers_to_dict(filename):
    ...
    for regel in regels:
        ticker_lst = regel.strip().split(':')
        #print(ticker_lst)
        if len(ticker_lst) == 2:
            bedrijf, ticker_symbol = ticker_lst
            ticker_dict[bedrijf] = ticker_symbol
...
def name_to_symbol(name, ticker_dict):
    ...
    ticker_symbol = ticker_dict.get(f'{name}', None)
...
def symbol_to_name(symbol, ticker_dict):
    ...
    for bedrijf, ticker_symbol in ticker_dict.items():
...
def development_code():
    ...
    tickers = tickers_to_dict('pe_7_2_testtickers.txt')
